...
public class ButtonLinksAction extends ActionSupport implements Preparable,ValidationAware{
	...
	/*public String trackURLReport(){
		...
		msgKey=TrackURLDB.getTrackCodeJsonData(getTrackCodeReport,Fields);
		if(export){
	...
	public void setFbeid(String fbeid) {
		this.fbeid = fbeid;
	}
	...
	public String execute(){
		...
		String username=DbUtil.getVal("select getMemberPref(mgr_id||'','pref:myurl','') as username from eventinfo where eventid=CAST(? AS BIGINT)", new String[]{eid});
		eventData.setServerAddress(EventDB.serverAdderess());
		eventData.setEventURL(EventDB.getEventURL(eid,username));
		eventData.setFaceBookTicketingPageURL(EventDB.getFaceBookTicketingPageURL(eid));
		eventData.setUserurl(EventDB.getUserURL(eid,eventData.getUsername()));
		eventData.setFacebookeventid(EventDB.getConfig(eid,"event.FBRSVPList.eventid",""));
		fbeid=eventData.getFacebookeventid();
		powertype=EventDB.getPowerType(eid);
		System.out.println("fbeid: "+fbeid);
		if("eventurl".equals(purpose)){
			return "success";
		}else if("buttons".equals(purpose)){
			return "success";
		}else if("widget".equals(purpose)){
			if(powertype.equals("RSVP")){
				String mgrId=ActionContext.getContext().getParameters().get("mgrId").toString();
				SpecialFeeDB.chekingSpecialFee(eid,mgrId,"150","RSVP Widget Code");
			}
			return "success";
		}else if("fb".equals(purpose)){
			return "fb";
		}else{
			return "success";
		}
	...
	public String customURL(){		
		eventData.setUserurl(EventDB.getUserURL(eid,eventData.getUsername()));
		eventData.setMgrId(EventDB.getUserId(eid));
		return "customurl";
	}
	public String saveFbeid(){
		System.out.println("save");	
		EventDB.updateFbeid(eid, fbeid);
		msgKey="success";
		return "ajaxmsg";
	}
	@Override
