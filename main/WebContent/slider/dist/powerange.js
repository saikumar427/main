...
Powerange.prototype.setRange = function(min, max) {
  if (typeof min === 'number' && typeof max === 'number' && !this.options.hideRange) {
	if(this.options.dollar){
    this.slider.querySelector('.range-min').innerHTML = '$0';
    this.slider.querySelector('.range-max').innerHTML = '$1,000';
	}
	
    else {
    	this.slider.querySelector('.range-min').innerHTML = '0';
    	this.slider.querySelector('.range-max').innerHTML = '10,000';
    }
  }
};
