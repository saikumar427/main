...
<s:hidden name="ticketData.processFeePaidBy" id="processfee"></s:hidden>
...
<s:else><s:hidden name="groupId"></s:hidden></s:else>
 <div class="col-md-6 col-sm-6 col-xs-12 bottom-gap">
   ...
   <div class="row bottom-gap sm-font options" >
    ...
    <select id="minqty" name="minqty" class="form-control"></select></div>
    <div class="col-md-1"></div> 
    <div class="col-md-1 col-xs-6 col-sm-3" style="margin-top: 10px;"> <s:text name="mt.max.lbl"/></div>
    <div class="col-md-2 col-xs-6 col-sm-3"><select id="maxqty" name="maxqty" class="form-control"></select></div>
    </div>
	...
	<label>
							<s:checkbox id="datetime"  name=""  cssClass="ticketsettings" checked="checked"> </s:checkbox></label>
    ...
    <div class="background-options row"  id="datetimeblock"> 
  ...
  <%--  <label for="name" class="col-md-2 col-sm-2 col-xs-2 control-label sm-font">Ticket sale date & time<span class="required">*</span></label> --%>
   ...
   <s:else>
   			...
   			<div class="col-md-1 col-sm-1 col-xs-6  sm-font" style="margin-top:10px;"><s:text name="mt.starts.lbl"/></div>
        ...
        <div class="col-md-1 col-sm-1 col-xs-6 sm-font" style="margin-top:10px;"><s:text name="mt.ends.lbl"/></div>
   ...
   <div class="row bottom-gap sm-font options">
	...
	<label>
							<s:checkbox id="description"  name="" cssClass="ticketsettings" /> </label>
 ...
 <%--    <div><s:property value="%{ticketData.ticketDescription}"></s:property>
    
    <s:if test="%{ticketData.ticketDescription!=''}"></s:if>display:block;<s:else>display:none;</s:else>
    </div> --%>
   ...
   <div class="background-options sm-font row" style="display:none;" id="paidbyblock">
  <!--  Attendee pays&nbsp; -->
   
   <div class="col-md-12">
   <s:text  name='ticketData.currency'></s:text>
  <s:textfield  id="processingfee" name="ticketData.processingFee" size="12" placeholder="%{getText('mt.enter.num.lbl')}" onfocus="getTicketLevelAgreeDiv();"  onkeyup="resetFee();"></s:textfield><br/>
   ...
   <s:if test="%{isrecurring==false}">
	...
	<label>
     ...
     <div>
  		<div class="col-md-3 col-sm-3 col-xs-12">
  		        <input class="tktichk waitlists" type="radio" <s:if test="%{ticketData.waitListType=='NOLIMIT' || ticketData.waitListType=='NO'}">checked="checked"</s:if> name="waitlist" id="noLimitWaitListType" value="NOLIMIT" onclick="getWaitListAgreeDiv('NOLIMIT');" >&nbsp;<s:text name="mt.waitlist.unlimited.lbl"/><br/>
				</div>
				<div class="col-md-5 col-sm-5 col-xs-12">
				<input class="tktichk waitlists" type="radio" <s:if test="%{ticketData.waitListType=='LIMIT'}">checked="checked"</s:if> name="waitlist" id="limitWaitListType" value="LIMIT" onclick="getWaitListAgreeDiv('LIMIT');" >&nbsp;<s:text name="mt.waitlist.limit.lbl"/>
				<s:textfield class="form-control" id="" name="ticketData.waitListLimit" theme="simple" size="12" placeholder="%{getText('mt.enter.num.lbl')}"  ></s:textfield><br/>
  		</div>
  		</div>
...
<div class="col-md-12 col-sm-12 col-xs-12 center" style="margin-top:10px;">
        <button type="button" class="btn btn-primary confirmaddticket"><s:text name="global.save.btn.lbl"/></button>
...
if(paramObj['ticketData.ticketDescription']!=undefined && paramObj['ticketData.ticketDescription']!=''){
	$('#description').iCheck('check');
	$('#descriptionblock').show();
}

if(paramObj['ticketData.processingFee']!=undefined && paramObj['ticketData.processFeePaidBy']=='processFeeAttendee' && Number(paramObj['ticketData.processingFee'])>0){
	$('#paidby').iCheck('check');
	$('#paidbyblock').show();
}

if(paramObj['ticketData.waitListType']!=undefined && paramObj['ticketData.waitListType']!='NO'){
	$('#waitlist').iCheck('check');
	$('#waitlistblock').show();
}


if(paramObj['ticketData.minQty']!=undefined){
	$('#minqty').val(paramObj['ticketData.minQty']);
}
if(paramObj['ticketData.maxQty']!=undefined)
	$('#maxqty').val(paramObj['ticketData.maxQty']);


if(Number(paramObj['ticketData.maxQty'])!=Number(paramObj['ticketData.totalTicketCount']) && Number(paramObj['ticketData.maxQty'])!=99){
	$('#quantity').iCheck('check');
	$('#quantityblock').show();
}
...
//if(paramObj['ticketData.waitListType']!=undefined && paramObj['ticketData.waitListType']!='NO')
	...
	/*  if(paramObj['ticketData.waitListType']=='NOLIMIT')
		 $('#noLimitWaitListType').iCheck('check');
	 else
		 $('#limitWaitListType').iCheck('check'); */
...
$('#description,#paidby,#waitlist,#quantity,#datetime').on('ifChecked',function(){
   ...
   $('#'+id+'spin').removeClass('original').addClass('down');
    ...
    if(id=='waitlist'){
      $('#waitlisttype').val($('.waitlists:checked').val());
      getWaitListAgreeDiv($('.waitlists:checked').val());
      }else if(id=='paidby')
...
if($('#ticketid').val()!=''){
	$('.confirmaddticket').html(props.global_save_btn);
}
