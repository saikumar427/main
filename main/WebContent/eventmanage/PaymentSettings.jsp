...
var ccNameValues={"paypal_pro":"Eventbee","braintree_manager":"Braintree","authorize.net":"Authorize.Net","stripe":"Stripe","payulatam":"PayU"};
...
<form id="paymentsettingsform" name="paymentsettingsform"
		...
		<hr>
			...
			<span id="ccbold" style="cursor: pointer;" class="checkbox-space">   <s:text name="ps.payment.cc.processing.lbl"/></span><span id="ccpindb"></span>    
			...
			<div  class="col-md-5" ><br>
				<div  id='otherblock' style='display: block'>
					...
					<%--  <s:property value="%{paymentData.paymentApprovalType}"/> --%>
					  <input type="checkbox" id="otherpaymentmode" name="payments"
							<s:if test="%{paymentData.paymentApprovalType == 'manual'}">checked='checked'</s:if> />
					<span id="paymentapprovalinfo" class="sm-font checkbox-space"><s:text name="ps.payment.approval.method"/></span>   
					<%-- <s:property value="%{paymentData.paymentApprovalType}"/> --%>
				</div>
		...
		<div class="form-group">
			<div class="row">
				...
				<div class="col-md-12 col-sm-12 col-xs-12 bottom-gap">
					<input type="checkbox" id="ccfeemode"   <s:if test="%{paymentData.ccprocessingfee == 'yes'}">checked='checked'</s:if>  />
						<span class="checkbox-space">
							<s:text name="ps.cc.fee.section.header"/>
							<i class="fa fa-info-circle info" style="cursor: pointer" id="creditcardinfo"></i>
						</span>
				</div>
				
				</div>
				
				<div class="background-options options" id="ccfeediv" style='<s:if test="%{paymentData.ccprocessingfee == 'yes'}">display:block;</s:if><s:else>display:none;</s:else>'>
				
				<div class="row ">
				
				<div class="col-md-12 col-sm-12 col-xs-12" id="processingtax">
					<table>
						<tr>
							<td>
	...
	<div class="form-group">
			...
			<s:if test="%{poweredbyEB=='yes' || eventbeevendor == 'braintree_manager'}">
				<div class="col-md-12 text-center" style="margin-top:-5px;">
					<button id="pSettSubmitBtn" class="btn btn-primary" onclick="updatePaymentSettings('');"><s:text name="%{getText('global.save.btn.lbl')}" /></button>
				</div>
			</s:if>
			<s:else>
				<div class="col-md-12 text-center" style="margin-top:-5px;">
...
$(document).ready(function(){
	...
	/* var ccmodetype='${paymentData.ccprocessingfee}';
	if(ccmodetype=='no'){
	$('#ccfee').val("0");
	$('#tax').val("0");
	} */
...
function updatePaymentSettings(val){
   ...
   if(j==1){
	   ...
	   var isAttChecked=$('#ccfeemode').is(':checked');
	  
	   if(myspecialfee!='' && isAttChecked==true)
	   {
		   ...
		   if(checkfee=='4.95' && (Number(document.getElementById('tax').value)>4.95 || Number(document.getElementById('ccfee').value)>0.5)){
			   ...
			   bootbox.confirm(props.ps_cc_processing_lbl1+"4.95%+"+csymbol+"0.5. "+props.ps_cc_processing_lbl2, function (result) {
				   if(result)
					   updateFees('4.95','0.5');
				   	   }); 
			   return;
			   ...
			   j=0;
			   bootbox.confirm(props.ps_cc_processing_lbl1+"2.9%+"+csymbol+"0.30. "+props.ps_cc_processing_lbl2, function (result) {	
				   if(result)
					   updateFees('2.9','0.30');
			   }); 	
		      return;
			   ...
			   j=0;
			   bootbox.confirm(props.ps_cc_processing_lbl1+"0%+"+csymbol+"0. "+props.ps_cc_processing_lbl2 , function (result) {
				   if(result)
					   updateFees('0','0');
			   }); 	
		      return;
   ...
   document.getElementById('payyoumarchantid').value=payyoumarchantid.replace(/^(?:\s)*/g,'').replace(/(?:\s)*$/g,'');
	...
	$('#pSettSubmitBtn').attr('disabled',true);
   ...
   if($('#otherpaymentmode').is(':checked'))
	...
	// alert($('#paymentsettingsform').serialize()); 
	$.ajax({
                        ...
                        $.ajax({
			  	      ...
			  	      else if(msg.indexOf("Payment Settings updated")>-1) 
			             {
			            	...
			            	// $('#paymentStatusMsg').addClass('alert alert-success');
			            	 $('#pSettSubmitBtn').attr('disabled',false);
			            	 updateSelectedCC();	
...
$('#ccdescdiv').show();
	...
	$('#ccdatablock').hide();
	 if(ccvendor.indexOf("paypal_pro")>-1 && ccDropMenu.indexOf("paypal_pro")>-1){
	...
	if(ccvendor.indexOf("braintree_manager")>-1 && ccDropMenu.indexOf("braintree_manager")>-1){
	...
	if(ccvendor.indexOf("authorize.net")>-1 && ccDropMenu.indexOf("authorize.net")>-1){
	...
	if(ccvendor.indexOf("stripe")>-1 && ccDropMenu.indexOf("stripe")>-1){
	...
	if(ccvendor.indexOf("payulatam")>-1 && ccDropMenu.indexOf("payulatam")>-1){
	  ...
	  $('#ccfeemode').on('ifChecked', function(event){ 
	    ...
	    $('.creditcardicheck').on('ifChecked', function(event){   
	    	...
	    	$('#ccdatablock').slideDown("slow");
	    	 ...
	    	 var ccvals=ccNameValues[$('#ccdropdown').val()];
	    	 if(ccvals==undefined)
	    		 ccvals="";
	    	 else
	    		 ccvals=": "+ccvals;
