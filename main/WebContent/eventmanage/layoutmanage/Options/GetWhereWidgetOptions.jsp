...
String data = "reset".equals(reset)?DBHelper.getWidgetOptions("0", widgetid):DBHelper.getWidgetOptions(eventid, widgetid);// DBHelper.getWidgetOptions("1", widgetid) get orginal datain DB.
...
$(function(){  
	...
	$('.savebtn').on('click',function(){
		//alert($('#chnglat').prop('checked'));
		//alert($.isNumeric($('#lat').val()));
		
		if(($('#chnglat').prop('checked')==true && $('#lat').val()!='' && $('#lng').val()!='') && (!$.isNumeric($('#lat').val()) || !$.isNumeric($('#lng').val())))
		{   alert(props.pg_where_lat_lag_lbl);
			   	return ;		  
		}
		var savebutton = $(this);
		savebutton.html(props.pg_widgets_saving_lbl);
		savebutton.attr('disabled','disabled');
		$.getJSON('SaveSettings?type=widgetOptions&eid=<%=eventid%>&widgetid=<%=widgetid%>&ref_title=Where',{
