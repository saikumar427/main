function BuildTextBoxCtrl(ControlData)
{
	...
	this.SetValue = function(textValue){
		...
		_inputbox.value=txtBoxValue;
	}
	this.GetValue = function(){
		return _inputbox.value;
	}
	...
	var _inputbox	= document.createElement("input");
	...
	_inputbox.type	= "text";
	_inputbox.id	= ControlData['Id'];
	_inputbox.name	= ControlData['Id'];
	_inputbox.size	= ControlData['textboxsize'];
	_inputbox.className = 'form-control';
	_inputbox.style.width = '80%';
	...
	_controlTD.appendChild(_inputbox);
	if((ControlData['DisplayStyle'] == '5') || (typeof(ControlData['DisplayStyle'])) == 'undefined' || ControlData['DisplayStyle']=='')
	{
		_labelTD.setAttribute("width","50%");
		_controlTD.setAttribute("width","50%");
		_ctrlTR.appendChild(_labelTD);
	}
	else if(ControlData['DisplayStyle'] == '6')
	{
