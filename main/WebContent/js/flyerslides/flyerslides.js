...
var Sliderman = new function(){
	...
	Sliderman.slider = function(parameters){
		...
		var Slider = {}, current, previous, prevImg, status = 'free', isHover = false, stopSlide=false, images = [], descriptions = [], links = [], ef = function(){};
		...
		if(description && !contentmode){
			...
			var descriptionContB = newElement('DIV'); partsCont.appendChild(descriptionContB);
			if(description.hide) {
				hide(descriptionCont.style);
				hide(descriptionContB.style);
			}
			
			var descriptionStl = {position: 'absolute', overflow: 'hidden', textAlign: 'left'};
			var descriptionStlB = {position: 'absolute', overflow: 'hidden', textAlign: 'left'};
			if(!description) description = [];
			description.position = validateOption(description.position, 'top,left,right,bottom')
			descriptionStl.background = description.background || 'white';
			descriptionStl.opacity = description.opacity || 0.5;
			descriptionStlB.background = description.background || 'white';
			descriptionStlB.opacity = description.opacity || 0.5;
			descriptionStl.width = description.position == 'top' || description.position == 'bottom' ? display.width : description.width || display.width*0.2;
			descriptionStlB.width =  display.width;
			descriptionStl.height = description.position == 'left' || description.position == 'right' ? display.height : description.height || display.height*0.2;
			descriptionStlB.height =  description.height || display.height*0.2;
			descriptionStl[description.position == 'bottom'?'bottom':'top'] = 0;
			descriptionStl[description.position == 'right'?'right':'left'] = 0;
			
			descriptionStlB['bottom'] = 0;
			descriptionStlB[description.position == 'right'?'right':'left'] = 0;
			
			var descBg = newElement('DIV', descriptionStl); descriptionCont.appendChild(descBg);
			var descBgB = newElement('DIV', descriptionStlB); descriptionContB.appendChild(descBgB);
			descriptionStl.opacity = 1; descriptionStl.background = '';
			descriptionStlB.opacity = 1; descriptionStlB.background = '';
			var desc = newElement('DIV', descriptionStl); descriptionCont.appendChild(desc);
			var descB = newElement('DIV', descriptionStlB); descriptionContB.appendChild(descB);
			
			var descriptionShow = function(){
				desc.innerHTML = '';
				setStyle(descriptionCont.style, 'visibility', 'hidden');
				descB.innerHTML = '';
				setStyle(descriptionContB.style, 'visibility', 'hidden');
				var value = descriptions[current];
				if(value){
					setStyle(descriptionCont.style, 'visibility', 'visible');
					setStyle(descriptionContB.style, 'visibility', 'visible');
					valueT = value.cloneNode(true);
					show(valueT.style);
					valueB = value.cloneNode(true);
					show(valueB.style);
					desc.appendChild(valueT);
					descB.appendChild(valueB);
					if(isHover) {
						show(descriptionCont.style);
						show(descriptionContB.style);
					}
				}
			}
		...
		if(navigation){
			...
			var navigationContB = document.getElementById(navigation.containerB);
			var a;
			...
			for(var i = 0; i < images.length; i++){
				...
				if(navigation.className) a.className = navigation.className;
				a.href = 'javascript:void(0);';
				a.id = parameters.container+'_SliderNavigationB'+i;
				a.onfocus = function(){this.blur();};
				a.onclick = function(){Slider.go(this.id.replace(/[^\d]+/g, ''));};
				navigationLinksB.push(a);
				navigationContB.appendChild(a);
			}
			...
			var navigationUpd = function(){
				for(var i = 0; i < navigationLinks.length; i++) {
					navigationLinks[i].className = navigationLinks[i].className.replace(/\bactive\b/g, '');
				}
				for(var i = 0; i < navigationLinksB.length; i++) {
					navigationLinksB[i].className = navigationLinks[i].className.replace(/\bactive\b/g, '');
				}
				navigationLinksB[current].className += ' active';
		...
		Slider.resume = function(){stopSlide=false;}
		Slider.stop = function(){stopSlide=true;}
		Slider.next = function(){if(!stopSlide) Slider.go(current + 1);}
		Slider.prev = function(){if(!stopSlide) Slider.go(current - 1);}
